...
public class MarkersPacket extends Packet {
    ...
    public void readPacketData(DataInput in) throws IOException {
        ...
        for (int i = 0; i < typesLength; i++) {
            ...
            for (int j = 0; j < markersLength; j++) {
                Marker marker = new Marker(in.readInt(), type, in.readUTF(), this.dimension, in.readInt(), in.readInt(), in.readBoolean());
    ...
    public int getPacketSize() {
        ...
        for (Map.Entry<String, Collection<Marker>> entry : this.markersByType.asMap().entrySet()) {
            ret = ret + Packet.getPacketSizeOfString(entry.getKey()) + 2;
            for (Marker marker : entry.getValue()) {
                ret = ret + 4 + Packet.getPacketSizeOfString(marker.getLabel()) + 4 + 4 + 1;
            }
        }
        return ret;
    }

...
public class ExportImageUtil {
    ...
    /** Beware that the background texture doesn't follow the Autotile getString. */
    ...
    public static void exportPngImage(DimensionData biomeData, DimensionMarkersData globalMarkers,
        ...
        if (showMarkers) {
            updateUnitsTotal += globalMarkers.getAllMarkers().collect(Collectors.toSet()).size();
            if (localMarkers != null) {
                updateUnitsTotal += localMarkers.getAllMarkers().collect(Collectors.toSet()).size();
            }
        }
        int updateUnits = 0;
